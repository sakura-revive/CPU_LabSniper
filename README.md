...
在`./config/forms`目录中，创建`xx.yaml`，或者原位复制`./config/forms/0.yaml`文件并将副本重命名为`xxx.yaml`。
...
- 请谨慎使用以下特殊功能，错误使用可能导致严重后果：
  ...
  - 用户身份
    ...
    - 在创建预约时，请确认目标用户有在当前仪器、当前时间段（`start`到`end`）预约的权限，如成功，则会显示目标用户在当前时间段预约了仪器；
...
- 其它可选功能：
  ...
  - 卡点预约
    - 若将`reserve_open_time`设置为具体的时间，则程序会在【该时间前10秒左右】这一时刻创建请求，并在【该时间前3秒到该时间】这一时间段之内提交数次请求，以期最大的预约成功率；
...
### 开始运行
- 不使用任何参数时，默认读取`./config/config.yaml`作为配置文件
(注意，你需要按照上述说明，自己创建这个文件)
```shell
python ./main.py
```
